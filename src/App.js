...
const App = () => {
  // --- 狀態變數 (模擬市場數據) ---
  // 1. 宏觀趨勢：台股加權指數 vs 200MA
  const [taiex, setTaiex] = useState(46465.2); // 預設帶入最新搜尋數值
  const [ma200, setMa200] = useState(38500); // 模擬當前 200MA 位階
  const [daysBelowMa, setDaysBelowMa] = useState(0);

  // 2. 市場情緒：VIX 恐慌指數
  const [vix, setVix] = useState(16.78); // 預設帶入最新搜尋數值
  const [daysVixAbove20, setDaysVixAbove20] = useState(0);

  // 3. 機會型槓桿指標：006208 核心資產回撤
  const [peak006208, setPeak006208] = useState(249.85); // 近期波段高點
  const [current006208, setCurrent006208] = useState(249.1); // 預設帶入最新搜尋數值
  ...
  const fetchLatestData = () => {
    ...
    // 模擬網路延遲與資料更新
    setTimeout(() => {
      // 這裡以當前搜尋到的最新市況為基準進行微調模擬
      const newTaiex = 46465.2 + (Math.random() * 100 - 50);
      setTaiex(Number(newTaiex.toFixed(1)));

      const newVix = 16.78 + (Math.random() * 0.5 - 0.25);
      setVix(Number(newVix.toFixed(2)));

      const new006208 = 249.1 + (Math.random() * 1 - 0.5);
      setCurrent006208(Number(new006208.toFixed(2)));
